...
func httpCmd(stop bool) {
	if stop {
		if _, err := console.Rpc.HttpClose(ctx, nil); err != nil {
			cLogger.Error("%v", err)
			return
		}
	}
	notif, _ := console.Rpc.HttpOpen(ctx, nil)
...
func httpsCmd(stop bool) {
	if stop {
		if _, err := console.Rpc.HttpsClose(ctx, nil); err != nil {
			cLogger.Error("%v", err)
			return
		}
	}
	notif, _ := console.Rpc.HttpsOpen(ctx, nil)
	log.NumericalLevel(cLogger, uint16(notif.LogLevel), notif.Msg)
}
